...
class BeginGameState(BaseState):
    def enter(self, **enter_params: Dict[str, Any]) -> None:
        ...
        # Validación: Garantizar jugabilidad desde el segundo cero
        if not self.board.has_possible_matches():
            self.board.shuffle_tiles()
            # Como aquí el tablero está oculto por el fade-in, 
            # forzamos las coordenadas físicas al instante sin usar tweens.
            for i in range(settings.BOARD_HEIGHT):
                for j in range(settings.BOARD_WIDTH):
                    self.board.tiles[i][j].x = j * settings.TILE_SIZE
                    self.board.tiles[i][j].y = i * settings.TILE_SIZE
