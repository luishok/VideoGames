...
def pickup_coin(
    ...
    # --- CONDICIÓN DE VICTORIA PARA EL NIVEL 1 ---
    current_level = getattr(player, "current_level", 1)
    
    if current_level == 1 and player.score >= 100:
        print("¡Meta de 100 puntos cumplida en el Nivel 1!")
        player.won = True
...
ITEMS: Dict[str, Dict[int, Dict[str, Any]]] = {
    ...
    "keys": {
        54: {
            ...
            "collidable": True,
            "consumable": True,
            "on_consume": lambda item, player: setattr(player, "won", True)
        }
    }
}