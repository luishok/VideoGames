...
class GameObject:
    ...
    def render(self, surface: pygame.Surface, offset_x: float = 0, offset_y: float = 0) -> None:
        current_texture = self.states[self.state].get("texture", self.texture_id)
        frame_index = self.states[self.state].get("frame", self.frame_index)

        texture_image = settings.TEXTURES[current_texture]

        try:
            surface.blit(
                texture_image,
                (self.x + offset_x, self.y + offset_y),
                settings.frame(current_texture, frame_index),
            )
        except KeyError:
            surface.blit(
                texture_image,
